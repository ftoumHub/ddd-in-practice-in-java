...
public class Atm extends AggregateRoot {
    ...
    private BigDecimal moneyCharged = ZERO;
    private static final BigDecimal COMMISSION_RATE = new BigDecimal("0.01");

    public String canTakeMoney(BigDecimal amount) {
        if(amount.compareTo(ZERO) <= 0) return "Invalid amount";

        if(moneyInside.getAmount().compareTo(amount) < 0) return "Not enough money";

        if(!moneyInside.canAllocateMoney(amount)) return "Not enough change";
    ...
    public BigDecimal calculateAmountWithCommission(BigDecimal amount) {
        ...
        if(lessThanCent.compareTo(ZERO) > 0) {
